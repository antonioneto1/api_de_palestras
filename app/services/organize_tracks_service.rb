class OrganizeTracksService
  ...
  def initialize(lectures)
    ...
    @coffee_break = {
      'lunch' => { 'hour' => '12h', 'title' => 'almoço', 'minutes' => '' },
      'network' => { 'hour' => '17h', 'title' => 'Evento de netWork', 'minutes' => '' }
    }
    @count_track = 0
  ...
  def process_item(item, track, current_minutes, only_numbers_from_minutes, schedules, schedules_array)
    if current_minutes >= @minutes_in_hours['12h'] && current_minutes <= @minutes_in_hours['13h']
      ...
      schedules_array << { schedule: schedules, title: item[:title], duration: item[:duration], track: track }
  ...
  def handle_lunch_break(item, track, current_minutes, schedules, schedules_array)
    ...
    new_minutes = @coffee_break['lunch']['minutes']

    schedules_array << { schedule: new_schedules, title: new_title, duration: new_minutes, track: track }
  ...
  def handle_networking_event(item, track, current_minutes, schedules_array)
    ...
    new_minutes = @coffee_break['network']['minutes']
  ...
  def add_networking_event_if_needed(track, schedules_array)
    last_item = schedules_array.reverse[0][:schedule].split('h')[0].to_i

    if last_item >= 15 && last_item != 17
      schedules_array << { schedule: @coffee_break['network']['hour'], title: @coffee_break['network']['title'], duration: @coffee_break['network']['minutes'], track: track }
    end
  end
